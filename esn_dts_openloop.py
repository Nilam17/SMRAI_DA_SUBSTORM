...
################################################################
# esn_dts_openloop.py: DTS-ESN (open-loop model)
...
class Output:
    def __init__(self, N_xu1, N_y, seed=0):
        '''
        ...
        '''
        np.random.seed(seed=seed)
        self.Wout = np.random.normal(size=(N_y, N_xu1))
        
    def __call__(self, xu1):
        '''
        param xu1: (N_x+N_u+1)-dim vector
        return: (N_y)-dim vector
        '''
        return np.dot(self.Wout, xu1)
...
class Tikhonov:
    def __init__(self, N_u, N_x, N_y, beta):
        '''
        param N_u: input dim
        param N_x: reservoir size
        param N_y: output dim
        ...
        '''
        self.beta = beta
        self.XU1_XU1T = np.zeros((N_x+N_u+1, N_x+N_u+1))
        self.D_XU1T = np.zeros((N_y, N_x+N_u+1))
        self.N_u = N_u
        self.N_x = N_x
        
    def __call__(self, d, xu1):
        xu1 = np.reshape(xu1, (-1, 1))
        d = np.reshape(d, (-1, 1))
        self.XU1_XU1T += np.dot(xu1, xu1.T)
        self.D_XU1T += np.dot(d, xu1.T)

    def get_Wout_opt(self):
        XU1_pseudo_inv = np.linalg.inv(self.XU1_XU1T \
                                     + self.beta*np.identity(self.N_x+self.N_u+1))
        Wout_opt = np.dot(self.D_XU1T, XU1_pseudo_inv)
        return Wout_opt
...
class ESN:
    def __init__(self, N_u, N_y, N_x, density=0.05, input_scale=1.0,
        ...
        self.Output = Output(N_x+N_u+1, N_y, seed=self.seed)
        self.N_u = N_u
        self.N_y = N_y
        self.N_x = N_x
